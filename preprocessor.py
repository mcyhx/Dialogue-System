...
def read_file(path, is_train=False):
    ...
    @param {type}
    path: file path
    is_train： whether the data is training data
    @return:list  include session_id, role, content
    '''
    chat = []
...
def clean(sent, sep='<'):
    ...
    sent: sententce
    sep: begin by'< 'or '[' 
    @return: string ,cleaning sentence
    '''
    sent = re.sub(r"[\s+\.\!\/_,$%^*(+\"\')]+|[+——()?【】“”！，。？、~@#￥%……&*（）]+",
                  "", sent)
    i = 0
...
def generate_data(filepath, save=True, to_file=None, pair=False):
    '''
    @description: process data， Q and A pair, or sentences merage 
    @param {type}
    file_path， original file 
    save, file save or not
    to_file， saved file path
    pair: process result as pair of Q and A
