...
class RANK(object):
    def __init__(self, do_train=True,  model_path=os.path.join(root_path, 'model/ranking/lightgbm')):
        ...
        if do_train:
            ...
            self.train = pd.read_csv(
                os.path.join(root_path, 'data/ranking/train.csv'),
                sep='\t',
                header=None,
                nrows=10000,
            ...
            self.test = pd.read_csv(
                os.path.join(root_path, 'data/ranking/test.csv'),
                sep='\t',
                header=None,
                quoting=csv.QUOTE_NONE,
                names=['question1', 'question2', 'label'])
    ...
    def trainer(self):
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            ...
            test_size=0.3,
            random_state=42)
        y_train = pd.to_numeric(y_train.values,errors = 'coerce').astype('int8')
        y_test = pd.to_numeric(y_test.values,errors = 'coerce').astype('int8')
